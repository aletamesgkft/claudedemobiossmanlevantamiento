...
const HEADER_BLOCK = {
  ask: "Ingrese los datos generales del levantamiento:\n**1) ID o nombre del hospital**\n**2) Nombre completo del responsable** (nombre, apellido paterno, apellido materno)",
  keys: ["idHospital", "nombre", "apPaterno", "apMaterno"],
  extractPrompt: `Extrae del texto estos campos como JSON puro sin backticks:
- idHospital: ID numérico o nombre del hospital
- nombre: nombre(s) de pila del responsable
- apPaterno: apellido paterno
- apMaterno: apellido materno
Si el usuario da un nombre completo como "Nestor Tobias Galvez Montes", separa: nombre="NESTOR TOBIAS", apPaterno="GALVEZ", apMaterno="MONTES".
Campos no mencionados = "". JSON puro, sin explicaciones.`
};
...
const EQUIP_BLOCKS = [
  {
    ...
    extractPrompt: `Extrae del texto estos campos como JSON puro sin backticks:
- ubicacion: debe ser uno de [${CAT_UBICACION.join(", ")}]. Normaliza "quirofano uno"→"QUIRÓFANO 1", "sala 1"→"SALA 1", etc.
- tipoEquipo: debe ser uno de [${CAT_TIPO.join(", ")}]. Normaliza "electrocauterizador"→"ELECTROCAUTERIO", "mesa de operaciones"→"MESA QUIRÚRGICA", "lampara"→"LÁMPARA QUIRÚRGICA", "monitor"→"MONITOR DE SIGNOS VITALES", "maquina anestesia"→"MÁQUINA DE ANESTESIA", "aspirador portátil"→"ASPIRADOR".
- marca: debe ser uno de [${CAT_MARCA.join(", ")}]. Normaliza "drager/dräger"→"DRAGER", "biosman"→"BIOSSMANN".
Si un valor no coincide con el catálogo pon "OTRO". JSON puro.`
  },
  {
    id: "block2",
    ...
    extractPrompt: `Extrae del texto estos campos como JSON puro sin backticks:
- modelo: texto libre (modelo del equipo)
- numSerie: texto libre (número de serie)
- anioManufactura: año en formato YYYY
Campos no mencionados = "". JSON puro.`
  },
  {
    id: "block3",
...
- propiedad: debe ser uno de [${CAT_PROPIEDAD.join(", ")}]. "del hospital/propio"→"HOSPITAL", "biossmann/biosman"→"BIOSSMANN".
- proveedor: debe ser uno de [${CAT_PROVEEDOR.join(", ")}]. Normaliza errores comunes.
Si un valor no coincide con el catálogo pon "OTRO". JSON puro.`
  },
  {
    id: "block4",
    ask: "**Riesgo características (0-5), riesgo funcionamiento (0-5) y fecha último servicio:**\n\nEscala: 0=Operativo, 1=Con limitaciones, 2=No adecuado, 3=Semi funcional, 4=No funcional, 5=No apto",
    keys: ["riesgoCaract", "riesgoFunc", "fechaServicio"],
    extractPrompt: `Extrae del texto estos campos como JSON puro sin backticks:
- riesgoCaract: número 0-5 (riesgo por características físicas)
- riesgoFunc: número 0-5 (riesgo por funcionamiento)
- fechaServicio: fecha en formato YYYY-MM-DD. Si dice "octubre 2025"→"2025-10-01". Si dice "no sé"→"".
Si el usuario dice dos números seguidos como "0 y 0" o "cero cero", el primero es riesgoCaract y el segundo riesgoFunc.
JSON puro.`
  },
  {
    id: "block5",
    ask: "Indique accesorios presentes y observaciones. Responda 'ninguno' si no aplica.",
    keys: ["accesorios", "observaciones"],
    extractPrompt: `Extrae del texto estos campos como JSON puro sin backticks:
- accesorios: texto libre describiendo accesorios presentes. "ninguno/nada/no"→""
- observaciones: texto libre con observaciones adicionales. "ninguna/nada/no"→""
JSON puro.`
  }
];
    ...
    try {
      ...
      if (phase === "header") {
        ...
        if (!fields || Object.values(fields).filter(Boolean).length === 0) {
          addBot("No se pudieron extraer los datos. Ejemplo de formato esperado:\n*\"1596, Nestor Tobias Galvez Montes\"*");
          setBusy(false);
          return;
        }
        setHeader((p) => ({ ...p, ...fields }));
        addBot(formatExtracted(fields) + "\n\n✓ Datos generales registrados. Vamos con el primer equipo.");
        setPhase("equip");
        setBlockIdx(0);
        setEquip({});
      ...
      if (phase === "equip" || phase === "equipSkipUbi") {
        ...
        if (phase === "equipSkipUbi" && blockIdx === 0) {
          prompt = `Extrae del texto estos campos como JSON puro sin backticks:
- tipoEquipo: debe ser uno de [${CAT_TIPO.join(", ")}]. Normaliza sinónimos.
- marca: debe ser uno de [${CAT_MARCA.join(", ")}]. Normaliza errores.
Si no coincide con catálogo pon "OTRO". JSON puro.`;
        }

        const fields = await extractFields(text, prompt, block.keys);
        if (!fields || Object.values(fields).filter(Boolean).length === 0) {
          addBot("No se pudieron extraer los datos. Proporcione los campos solicitados.");
          setBusy(false);
          return;
        }
